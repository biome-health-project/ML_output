...
df<-alldf%>%
  dplyr::select(IMG_ID, ID_unique, Site,Camera ,Season,  SumAnimal, MeanConf)%>%
  gsub("_1.JPG", "", IMG_ID) %>% 
  gsub("_0.JPG", "", IMG_ID) %>% 
  distinct() %>%
  ungroup()
...
df_all<-vroom::vroom("Kenya_Animal_No_Animal1.csv")
...
image$ID_unique<-image$filepath %>% 
                    gsub("M:/biome_health_project_files/country_files/kenya/raw_data//", "", .) %>%                        gsub(".JPG", "", . )%>% 
                    gsub("mara_november", "/november", .) %>% 
                    gsub("_october", "/october", .) %>% 
                    gsub("_repaired", "", .) %>% 
                    gsub("/100_BTCF","", . )
...
df_image<-merge(image[,c("ID_unique", "new_file_structure")], df_sub, by = "ID_unique")


not_in<-image[!image$ID_unique %in% df_sub$ID_unique,]
